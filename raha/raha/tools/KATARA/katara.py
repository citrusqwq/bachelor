...
def domain_spec_col_type(
    data, col, domain_specific_types, col_2_errors_repair, type_coverage, ignore_null
):
...
def domain_spec_colpair(
    data, i, j, rel2sub2obj, col_2_errors_repair, pair_coverage, ignore_null
):
    for rel in rel2sub2obj:
        ...
        if coverage >= pair_coverage:
            col_2_errors_repair.update(
                tempdict
            )  # adds the errors found to the final output
