...
class Dataset:
    ...
    def __init__(self, dataset_dictionary):
        ...
        if "clean_path" in dataset_dictionary:
            ...
            self.clean_dataframe = self.read_csv_dataset(
        ...
        if "repaired_path" in dataset_dictionary:
            ...
            self.repaired_dataframe = self.read_csv_dataset(
    ...
    def read_csv_dataset(self, dataset_path):
        ...
        dataframe = pandas.read_csv(
            ...
            low_memory=False,
        ).applymap(self.value_normalizer)
        return dataframe
    ...
    def write_csv_dataset(dataset_path, dataframe):
        ...
        """
        dataframe.to_csv(
            dataset_path, sep=",", header=True, index=False, encoding="utf-8"
        )
    ...
    def get_dataframes_difference(dataframe_1, dataframe_2):
        ...
        difference_dataframe = dataframe_1.where(
            dataframe_1.values != dataframe_2.values
        ).notna()
        for j in range(dataframe_1.shape[1]):
            for i in difference_dataframe.index[
                difference_dataframe.iloc[:, j]
            ].tolist():
    ...
    def create_repaired_dataset(self, correction_dictionary):
        ...
        for cell in correction_dictionary:
            self.repaired_dataframe.iloc[cell] = self.value_normalizer(
                correction_dictionary[cell]
            )
    ...
    def get_data_cleaning_evaluation(
        self, correction_dictionary, sampled_rows_dictionary=False
    ):
        """
        ...
        if sampled_rows_dictionary:
            actual_errors = {
                (i, j): actual_errors[(i, j)]
                for (i, j) in actual_errors
                if i in sampled_rows_dictionary
            }
        ed_tp = 0.0
        ec_tp = 0.0
        output_size = 0.0
...
if __name__ == "__main__":
    ...
    error_dict = d.get_actual_errors_dictionary()
    key_list = error_dict.keys()
    column_error_dict = {}
    for cell in key_list:
        if cell[1] in column_error_dict:
            column_error_dict[cell[1]] += 1
        else:
            column_error_dict[cell[1]] = 1

    print(column_error_dict)
    print(key_list)
########################################
