...
def get_selected_strategies_via_historical_data(
    ...
    """for strategy_file in os.listdir(sp_folder_path):
        strategy_profile = pickle.load(
            open(os.path.join(sp_folder_path, strategy_file), "rb"))
        strategies_output[strategy_profile["name"]
                          ] = strategy_profile["output"]
        strategies_runtime[strategy_profile["name"]
                           ] = strategy_profile["runtime"]"""
    good_strategies_algorithm_and_configurations = []
    od_strategy_dict = {}
    # strategy_col_cnt_dict = {}
    ...
    for a in d.dataframe.columns.tolist():
        ...
        for sn, ss in sorted_strategies:
            # if cnt == strategy_col_cnt_dict[a]:
            #     break

            sn_list = ast.literal_eval(sn)

            if sn_list[0] == "PVD":
                column_name = sn_list[1][0]
                letter = sn_list[1][1]

                ndp_folder_path = os.path.join(d.results_folder, "dataset-profiling")
                ncp = pickle.load(
                    open(
                        os.path.join(ndp_folder_path, column_name + ".dictionary"), "rb"
                    )
                )
                character_dict_keys = ncp["characters"].keys()

                if letter not in character_dict_keys:
                    continue
            ...
            if sn_list[0] == "OD":
                od_strategy_dict.setdefault(str(sn_list), []).append(a)
            else:
                configuration = [d] + sn_list
                if configuration not in good_strategies_algorithm_and_configurations:
                    good_strategies_algorithm_and_configurations.append(configuration)

            # cnt += 1

        # strategy_col_cnt_dict[a] = len(good_strategies)
    ...
    for key, value in od_strategy_dict.items():
        configuration = [d] + ast.literal_eval(key) + [value]
        good_strategies_algorithm_and_configurations.append(configuration)
    print("Filtering runtime: {}".format(time.time() - start_time))
    # print(strategy_col_cnt_dict)

    return good_strategies_algorithm_and_configurations, selected_strategy_profiles
